/*
 * $Id: gfsd.c 4016 2008-09-10 16:04:16Z tatebe $
 */
// if define debug, gfsd will stop when finished
//#define GFSD_DEBUG
//#define BLOB_ID 1
//#define BLOB_REPLICA_COUNT 1
//#define BLOB_PAGE_SIZE 4194304//2097152//1048576//67108864//1048576 //8 page size = 1MB
//#define BLOB_CONFIGURATION "/media/Programfiles/opt/blobseer/release-0.1/lib/test.cfg"
#include "blobseer.h"
#include "uthash.h" //implementing hashtable
//-----------------------------------
...
struct blob_hashtable {
    char key_ino_gen[43];             /* key length 43 template[] = "/data/00112233/44/55/66/778899AABBCCDDEEFF";*/
    int blob_id;
    UT_hash_handle hh;         /* makes this structure hashable */
} *blob_file_table = NULL;
...
file_table_add(gfarm_int32_t net_fd, int local_fd, int flags)
{
	...
	blob = &(fe->blob);
	if(!blob_setid(&env, local_fd, blob))
			{
                          gflog_debug("Error, cannot setid in file_table_add!\n");
	...
	/* XXX FIXME st_mtimespec.tv_nsec */
        blob_getlatest(blob);
	fe->size = blob_getsize(blob,blob->latest_version);
        fe->active_version = 0;
        //        free(blob);
}
...
file_table_close(gfarm_int32_t net_fd)
{
	...
	blob_free(&env, &(file_table[net_fd].blob));
...
#define DIRLEVEL 5 /* there are 5 levels of directories in template[] */
	...
	length = sizeof(template);
	
	GFARM_MALLOC_ARRAY(p, length);
	if (p == NULL) {
		fatal("%s: no memory for %d bytes", diag, length);
	}
	snprintf(p, length, "/data/%08X/%02X/%02X/%02X/%02X%08X%08X",
...
{
	//TRAN need to convert path to blob_id, at present, blob_id = 0 
  	return 1;
	blob_t blob;
		
	if ((flags & O_CREAT) == 0)
	{
		struct blob_hashtable *bh;	
		HASH_FIND_STR(blob_file_table, path, bh);
			
		if(!bh || !blob_setid(&env, bh->blob_id, &blob))
		{
						gflog_debug("Error, cannot setid!\n");
						errno = ENOENT;
				        return -1;
		}
	}
	else
	{
		...
		if (!blob_create(&env, BLOB_PAGE_SIZE, BLOB_REPLICA_COUNT, &blob)) {
				gflog_debug("Error, cannot alloc!\n");
				errno = ENOENT;
		        return -1;
		    }
		struct blob_hashtable *bh;
		bh = malloc(sizeof(struct blob_hashtable));
		strcpy(bh->key_ino_gen, path);
		bh->blob_id = blob.id;
		HASH_ADD_STR(blob_file_table,key_ino_gen,bh ); //key_ino_gen keyfield
...
gfs_server_open_local(struct gfp_xdr *client)
{
	...
	if (gfs_server_open_common(client, "open_local", &net_fd, &local_fd) !=
		...
		return;
	//TRAN begin
	dummy = local_fd; //send local_fd via dummy field
	//TRAN end
	...
	if ((fe = file_table_entry(fd)) == NULL)
		...
		//}
			...
			stat_is_done = 1;
                        blob_getlatest(blob);
			fe->size=blob_getsize(blob,blob->latest_version);
			file_table_set_read(fd);	//moi them vao close_local
		//	if (fe->flags & O_RDWR || fe->flags & O_WRONLY)
...
gfs_server_pread(struct gfp_xdr *client)
{
	...
	struct file_entry *fe;
		if ((fe = file_table_entry(fd)) == NULL)
		{
				save_errno = -1; // doi thanh -1
				goto blob_read_error;
		}

        gfarm_int64_t blob_size; 
        blob_t *blob = &(fe->blob);
        if ( fe->active_version == 0 )
          {
            blob_getlatest(blob);
          ...
          {
            blob_size = blob_getsize(blob,fe->active_version);	//get the blob size of a specified version
          }
	...
	if(blob_size > size + offset)
	{
		...
		char * blob_buffer;
		blob_buffer = malloc(new_size);	//buffer

                if ( fe->active_version == 0 )
                  rv = blob_read(blob,blob->latest_version,new_offset,new_size,blob_buffer);
                else
                  rv = blob_read(blob,fe->active_version,new_offset,new_size,blob_buffer);

		if (rv !=0 )
                  {
                    memcpy(buffer, blob_buffer+rdoff,size);
	...
	gfs_server_put_reply_with_errno(client, "pread", rv == 0 ? save_errno : 0,
	    "b", rv, buffer);
}
...
gfs_server_pwrite(struct gfp_xdr *client)
{
	...
	{
			save_errno = -1; // doi thanh -1
			goto blob_write_error;
	}

		blob_t *blob = &(fe->blob);
                gfarm_int64_t blob_size;
                blob_getlatest(blob);
                blob_size = blob_getsize(blob,blob->latest_version);	//get the blob size of a specified version
...
//TRAN beginning
void
gfs_server_fget_version_count(struct gfp_xdr *client)
{
	int fd;
	size_t nversion=-1;
	int save_errno = 0;

	gfs_server_get_request(client, "fget_version_count", "i", &fd);

	struct file_entry *fe;
	if ((fe = file_table_entry(fd)) == NULL)
	{
          save_errno = -1; // doi thanh -1
          goto fget_version_count_error;
	}
        blob_t *blob = &(fe->blob);
        blob_getlatest(blob);
        nversion = blob->latest_version;
 fget_version_count_error:
	gfs_server_put_reply_with_errno(client, "fget_version_count", save_errno, "i", nversion);
  
}
void
gfs_server_fset_version(struct gfp_xdr *client)
{
	int fd;
	size_t nversion = 0;
	int save_errno = -1;

	gfs_server_get_request(client, "fset_version", "ii", &fd, &nversion);

	struct file_entry *fe;
	if ((fe = file_table_entry(fd)) == NULL)
		goto fset_version_error;

	blob_t *blob = &(fe->blob);
    if (blob_getlatest(blob))
    {
    	if(nversion <= -1 || nversion > blob->latest_version)
    		fe->active_version = blob->latest_version;
    	else
    		fe->active_version = nversion;
    } else
    	goto fset_version_error;

    save_errno =0; // no error

 fset_version_error:
	gfs_server_put_reply_with_errno(client, "fset_version", save_errno, "i", fe->active_version);
	...
	enum gfarm_auth_id_type peer_type)
{
	...
	local_path(ino, gen, diag, &path);
	local_fd = open_data(path, O_RDONLY);	//blob_id;
	free(path);
	if (local_fd < 0) {
		error = gfarm_errno_to_error(errno);
		goto send_eof;
	}
	//TRAN must create a blob handle here
	//     	blob_t blob;
	//	if (!blob_setid(&env, BLOB_ID, &blob)) {
	//		gflog_debug("Error, cannot set blob_id on replica recv!\n");
	//		return;
	/* data transfer */
	...
	char *src_canonical_hostname, char *src_if_hostname)
{
	...
	if (e != GFARM_ERR_NO_ERROR) {
		...
		if (e != GFARM_ERR_NO_ERROR) {
			...
			//TRAN-----------------------------------------------------------
				{
				char msg[200] = "TRAN: in gfs_server_replicate_file_sequential_common open(path, O_WRONLY|O_CREAT|O_TRUNC, mode);";
			...
			if (fd < 0) {
				...
				//TRAN-----------------------------------------------------------
						{
						char msg[200] = "TRAN: in gfs_server_replicate_file_sequential_common close(fd)";
						char tmp[200];
						sprintf(tmp, ":fd:%d", fd);
						strcat(msg,tmp);
						gflog_debug(msg);
	...
	char *src_canonical_hostname, char *src_if_hostname)
{
	...
	//TRAN-----------------------------------------------------------
		{
		char msg[200] = "TRAN: in gfs_server_replicate_file_parallel_common open(path, O_WRONLY|O_CREAT|O_TRUNC, mode);";
		char tmp[200];
		strcat(msg,path);
		sprintf(tmp, ":fd:%d", ofd);
		strcat(msg,tmp);
		gflog_debug(msg);
		}
	//TRAN-----------------------------------------------------------
...
finish_ofd:
	...
	//TRAN-----------------------------------------------------------
			{
			char msg[200] = "TRAN: in gfs_server_replicate_file_parallel_common close(ofd);";
			char tmp[200];
			sprintf(tmp, ":ofd:%d", ofd);
			strcat(msg,tmp);
			gflog_debug(msg);
			}
	//TRAN-----------------------------------------------------------
...
#define N_EXTRA_ENV	4	/* user_env, home_env, shell_env, path_env */
	...
	if (use_xauth_env) {
		...
		//TRAN-----------------------------------------------------------
			{
			char msg[200] = "TRANclose avant: in gfs_server_command popen(xauth_command, \"w\")";
			strcat(msg,xauth_command);
			char tmp[200];
			sprintf(tmp, ":fp:%d", fp);
			strcat(msg,tmp);
			gflog_debug(msg);
		...
		//TRAN-----------------------------------------------------------
				{
				char msg[200] = "TRAN: in gfs_server_command pclose(fp);";
				char tmp[200];
				sprintf(tmp, ":fp:%d", fp);
				strcat(msg,tmp);
				gflog_debug(msg);
...
server(int client_fd, char *client_name, struct sockaddr *client_addr)
{
	...
	for (;;) {
		...
		case GFS_PROTO_OPEN_LOCAL:
			// This RPC call now serves for BlobSeer direct access mode. 
			// We may change it to an appropriate name later
			gfs_server_open_local(client); break;
		case GFS_PROTO_OPEN:
			
			gfs_server_open(client); break;
		case GFS_PROTO_CLOSE:
			
			gfs_server_close(client); break;
		case GFS_PROTO_PREAD:
							
			gfs_server_pread(client); break;
		case GFS_PROTO_PWRITE:
			
			gfs_server_pwrite(client); break;
		case GFS_PROTO_FTRUNCATE:
			
			gfs_server_ftruncate(client); break;
		case GFS_PROTO_FSYNC:
			
			gfs_server_fsync(client); break;
		case GFS_PROTO_FSTAT:	
			
			gfs_server_fstat(client); break;
		case GFS_PROTO_CKSUM_SET:
			
			gfs_server_cksum_set(client); break;
		case GFS_PROTO_STATFS:
			
			gfs_server_statfs(client); break;
			//TRAN begin
                        //RPC calls for versioning capability
                case GFS_PROTO_FGET_VERSION_COUNT:
			//get latest version of an opened file
                        gfs_server_fget_version_count(client); break;
                case GFS_PROTO_FSET_VERSION:
			//set active file version of an opened file
                        gfs_server_fset_version(client); break;
                       //TRAN end
#if 0 /* not yet in gfarm v2 */
...
//TRAN begin
//This test function is for verifying if we can call BlobSeer API inside the gfsd daemon
...
main(int argc, char **argv)
{
	...
	//TRAN begin-----------------------------------------------------------
	//Initializing the env of BlobSeer
	...
	//TRAN begin-----------------------------------------
	//Finalizing the env when stopping the gfsd daemon
	blob_finalize(&env);
	//TRAN end-----------------------------------------
