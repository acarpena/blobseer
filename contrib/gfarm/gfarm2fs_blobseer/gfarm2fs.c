...
#define BLOB_VERSION "blob.active_version"
#define BLOB_LASTEST_VERSION "blob.lastest_version"
#define EXTENDED_ATTRIBUTES "blob.active_version\0blob.lastest_version"
#include "gfioctl.h"
//TRAN end
#ifdef HAVE_SETXATTR
#include <sys/xattr.h>
...
{
  if (!strcmp(name,BLOB_VERSION))
    {
      char buffer[64];
      snprintf(buffer, 64, "%i",64);
      if (size == 0)
  ...
  else if (!strcmp(name,BLOB_LASTEST_VERSION))
    {
      char buffer[64];
      snprintf(buffer, 64, "%i",128);
      if (size == 0)
        return strlen(buffer);
      if (strlen(buffer) > size)
        return -ERANGE;
...
{
  int length;
  //char buffer[64]="blob.active_version";
  length =sizeof(EXTENDED_ATTRIBUTES);
  if (size == 0)
    return length;
  if ( length > size)
    return -ERANGE;
  memcpy(list,EXTENDED_ATTRIBUTES,length);
  return length;
  /* XXX FIXME */
	return (-ENOSYS);
#if 0
...
static int
gfarm2fs_ioctl(const char *path, int cmd, void *arg,
                      struct fuse_file_info *fi, unsigned int flags, void *data)
{
  ...
  //e = gfs_pio_set_version(gf,60);
  //e = gfs_pio_get_version_count(gf,&realoffset);
  //e = gfs_pio_set_version(gf,60);
  //e = gfs_pio_get_version_count(gf,&realoffset);
 	gfarm_error_t e;
        size_t nversion;
  ...
  switch (cmd) {
  case BLOB_GET_ACTIVE_VERSION:
    
    e = gfs_get_active_version(get_filep(fi),&nversion);
    *(size_t *)data = nversion;
    if (e == GFARM_ERR_NO_ERROR) return 0;
    else return -EINVAL;

  case BLOB_SET_ACTIVE_VERSION:
    
    e = gfs_set_version(get_filep(fi),*(size_t *)data);
    if (e == GFARM_ERR_NO_ERROR) return 0;
    else return -EINVAL;
    
  case BLOB_GET_VERSION_COUNT:

    e = gfs_get_latest_version(get_filep(fi),&nversion);
    *(size_t *)data = nversion;
    if (e == GFARM_ERR_NO_ERROR) return 0;
    else return -EINVAL;
  }
...
int main(int argc, char *argv[])
{
	...
	return (fuse_main(argc, nargv, &gfarm2fs_oper,NULL));
}
