/* Derived from the NIST statistical suite by 
   Andre Seznec IRISA/INRIA March 2002, updated June 2002 */
/* limitations:  
tailored to work on sequences larger than 1 Mbyte and smaller than 256 Mbytes*/
...
RandomExcursions (int n, int *ARRAY)
{
  ...
  double pi[10][6] =
    ...
    0.00000000, .5000000000, .2500000000, .1250000000,
    .06250000000, .03125000000, .0312500000, .7500000000,
    .06250000000, .04687500000, .03515625000, .02636718750,
    .0791015625, .8333333333, .02777777778, .02314814815,
    .01929012346, .01607510288, .0803755143, .8750000000,
    .01562500000, .01367187500, .01196289063, .01046752930,
    .0732727051
  };
...
{
  int i, j, k, l, r, blockSize, seqLength;
...
psi2 (int m, int n, int *ARRAY, int PT)
{
  ...
  P = MYCALLOC (65536, sizeof (unsigned int));
...
/*  Complex  sum, *f;*/
  ...
  ifac = (double *) MYCALLOC (15, sizeof (double));
...
/* since we are not interested in short sequences we used only 10000*/
  ...
  double k[7] = { 10, 11, 12, 13, 14, 15, 16 };
  double K = 6;
...
create_matrix (int M, int Q)
{
  ...
  if ((matrix = (BitField **) MYCALLOC (M, sizeof (BitField *))) == NULL)
  ...
  else
    {
      for (i = 0; i < M; i++)
	{
	  if ((matrix[i] = MYCALLOC (Q, sizeof (BitField))) == NULL)
	    {
	      fprintf (stderr,
