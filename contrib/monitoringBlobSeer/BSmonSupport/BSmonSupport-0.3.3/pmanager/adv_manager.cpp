 ...
 */
rpcreturn_t adv_manager::get(const rpcvector_t &params, rpcvector_t & result, const std::string &sender) {
    if (params.size() != 2) {
	...
	adv_table_by_info &info_index = adv_table.get<tinfo>();
	{
		...
		for (int i = 0; i < (int)replica_no && found; i++) {
			...
			for (int j = (int)adv_list.size() - 1; 
			    ...
			    if (ai->id.first == adv_list[j].get_host() && 
				ai->id.second == adv_list[j].get_service() ) {
	...
	if (adv_list.size() != no_providers) {
        ...
        ////////////////
        exec_hooks(PUBLISHER_GET, buffer_wrapper(adv_list, true), sender);
...
/// hooks that trigger the sending of monitoring messages
////////////////
void adv_manager::exec_hooks(const boost::int32_t rpc_name, buffer_wrapper prov_list, const std::string &sender) {
    for (update_hooks_t::iterator i = update_hooks.begin(); i != update_hooks.end(); ++i)
        (*i)(rpc_name, monitored_params_t(prov_list, sender));
}

void adv_manager::add_listener(update_hook_t hook) {
