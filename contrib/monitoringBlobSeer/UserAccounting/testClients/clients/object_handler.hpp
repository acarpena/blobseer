...
#include "client/range_query.hpp"
...
protected:
    ...
    bool virtual exec_write(boost::uint64_t offset, boost::uint64_t size, char *buffer, bool append = false);
...
void rpc_write_callback(boost::dynamic_bitset<> &res, unsigned int k, unsigned int rc, 
			const rpcreturn_t &error, const rpcvector_t &) ;
void rpc_result_callback(bool &res, const rpcreturn_t &error, const rpcvector_t &) ;
template <class T> void rpc_get_serialized(bool &res, T &output, const rpcreturn_t &error, const rpcvector_t &result)
{
