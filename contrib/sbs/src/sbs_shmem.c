...
static int init_data_field(sbs_cmdq q)
{
	...
	// setting input and output index
	q->data->input = 0;
	q->data->output = 0;
	// unlocking
...
{
	int rc;
	// this is the number of semaphores to create
	int sem_nbr = 3 + SBS_CLIENT_NBR*SEM_PER_CLIENT;
	// keys associated with shared objects
	key_t semkey, shmkey;
	sbs_shmem mem = (sbs_shmem)malloc(sizeof(struct sbs_shmem_s));
	
	semkey = generate_key();//ftok(SEMKEYPATH,SEMKEYID);
	if(semkey == (key_t)-1)
	{
		printf("sbs_shmem_create: ftok failed\n");
		return NULL;
	}
	shmkey = generate_key();//ftok(SHMKEYPATH,SHMKEYID);
	if(shmkey == (key_t)-1)
	{
		printf("sbs_shmem_create: ftok failed\n");
		return NULL;
