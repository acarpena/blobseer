...
void sbs_server_main(MPI_Comm sbsc) 
{
	...
	int need_exit = 0;
	int sbsc_cmd; // last cmd received through mpi
	sbs_buffer buffer;

	// initialize shmem segment and cmdq
	...
	while(!need_exit) {
		// start waiting (non-blocking) for a MPI request from
		// any other sbs server
		...
		int r = 0;
		MPI_Status status;	
		// check if there is a message from other
		// sbs server to process
		if(sbsc_req != MPI_REQUEST_NULL)
			MPI_Test(&sbsc_req, &flag, &status);
		else flag = 0;
		// if we received a cmd, process it
		if(flag) r = sbs_server_process_from_sbss(sbsc_cmd,
			                                  &status,sbsc);
		need_exit = need_exit || (r != 0);

		// process commands from shared memory cmdq
...
int sbs_server_process_from_sbss(int cmd, MPI_Status *status, MPI_Comm comm) 
{
	// this function is made only for point-to-point comm
	// between two particular instances of SBS, but it shouldn't
	// be used in our current protocols
