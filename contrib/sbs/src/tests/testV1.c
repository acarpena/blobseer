...
void first_actor() {
	sbs_mailbox mb = sbs_mailbox_create("./sbs_mailbox.c",66);
	if(mb == NULL) exit(0);
	printf("task 0 : mailbox created.\n");
	
	printf("task 0 : sending key %d...\n",mb->key);
	MPI_Send(&(mb->key), 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
...
void second_actor() {
	...
	sbs_mailbox mb = sbs_mailbox_init(mem_key);
